...
public class ChecadorLetras : MonoBehaviour
{
    ...
    public GameObject figuraInicial;
    public Transform posicionPalabra;
    ...
    public event Action destroyDrawLines;
    ...
    void Start()
    {
        ...
        vectorCentro = posicionPalabra.transform.position;
        vectorCentro.z = -2;
        ...
        SpriteRenderer sprite = figuraInicial.GetComponent<SpriteRenderer>();
        sprite.sprite = Resources.Load("Textures/" + EstadoJuego.estadoJuego.palabra + "R", typeof(Sprite)) as Sprite;

        if (p.Equals("amarillo") || p.Equals("rojo") || p.Equals("azul") || p.Equals("anaranjado") || p.Equals("verde") || p.Equals("rosa") || p.Equals("negro"))
        {
            audioClip = Resources.Load("Sonidos/Colores/" + p, typeof(AudioClip)) as AudioClip;
            AudioSource sonido2 = GameObject.Find("sonido").GetComponent<AudioSource>();
            GameObject.Find("btnAudio").GetComponent<AudioSource>().clip = audioClip;
            reproducirSonido(sonido2, audioClip);
        }
        else
        {
            audioClip = Resources.Load("Sonidos/Figuras/" + p, typeof(AudioClip)) as AudioClip;
            AudioSource sonido2 = GameObject.Find("sonido").GetComponent<AudioSource>();
            GameObject.Find("btnAudio").GetComponent<AudioSource>().clip = audioClip;
            reproducirSonido(sonido2, audioClip);
        }
    ...
    public void SeHizoContacto(String idString)
    {
        ...
        if (idRepetido != id)
        {
            ...
            if (aciertos == aciertosNecesarios)
            {
                ...
                if (i >= 0)
                {
                    if (tipoLetra.Equals("dosTrazos"))
                    {
                        drawLines[i].Alto((float)0.3);
                    }
                    else
                    {
                        drawLines[i].Alto((float)0.3);
