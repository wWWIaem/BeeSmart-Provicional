...
public class Checador : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (palabra.Equals("amarillo") || palabra.Equals("rojo") || palabra.Equals("azul") || palabra.Equals("anaranjado") || palabra.Equals("verde") || palabra.Equals("rosa") || palabra.Equals("negro"))
        {
            sonido = Resources.Load<AudioClip>("Sonidos/Colores/" + palabra);
            //Esta linea hace que se escuche doble el audio, por eso se removio, porque en el cargar escena se toma el audio y se reproduce cuando se carga
            //GetComponent<AudioSource>().PlayOneShot(sonido);
            figura = Resources.Load("Prefab/colorEjercicio") as GameObject;

            UnityEngine.Debug.Log(palabra);
            audioClip = Resources.Load("Sonidos/Colores/" + palabra, typeof(AudioClip)) as AudioClip;
            sonido2 = GameObject.Find("sonido").GetComponent<AudioSource>();
            reproducirSonido(sonido2, audioClip);
        ...
        else
        {
            ...
            audioClip = Resources.Load("Sonidos/Figuras/" + palabra, typeof(AudioClip)) as AudioClip;
            sonido2 = GameObject.Find("sonido").GetComponent<AudioSource>();
            reproducirSonido(sonido2, audioClip);
            GameObject.Find("btnAudio").GetComponent<AudioSource>().clip = audioClip;
        }
    ...
    void EjercicioExitoso()
    {
        if (terminar)
        {
            ...
            string palabra = EstadoJuego.estadoJuego.palabra;
            /*Sprite sprite = Resources.Load("Textures/" + palabra + "R", typeof(Sprite)) as Sprite;

            
            if (sprite != null)
            {
                UnityEngine.Debug.Log("Textures/" + palabra + "R");
            }
            else
            {
                UnityEngine.Debug.Log("esta madre es nula: " + "Textures/" + palabra + "R");
            }
            */

            if (palabra.Equals("amarillo") || palabra.Equals("rojo") || palabra.Equals("azul") || palabra.Equals("anaranjado") || palabra.Equals("verde") || palabra.Equals("rosa") || palabra.Equals("negro"))
            {
                GameObject.Find("colorpunteado").GetComponent<SpriteRenderer>().sprite = Resources.Load("Textures/" + palabra + "R", typeof(Sprite)) as Sprite;
            }
            else
            {
